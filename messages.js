...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'text')) {
        ...
        const zipChunks = [];
        for await (const chunk of stickerPackUpload.encWriteStream) zipChunks.push(chunk);
        const zipEncBuffer = Buffer.concat(zipChunks);
        const stickerPackUploadResult = await options.upload(zipEncBuffer, {
            fileEncSha256B64: stickerPackUpload.fileEncSha256.toString('base64'),
            mediaType: 'sticker',
            timeoutMs: options.mediaUploadTimeoutMs
        });
        const thumbUpload = await encryptedStream(coverBuffer, 'image', {
            logger: options.logger,
            opts: options.options
        });
        const thumbChunks = [];
        for await (const chunk of thumbUpload.encWriteStream) thumbChunks.push(chunk);
        const thumbEncBuffer = Buffer.concat(thumbChunks);
        const thumbUploadResult = await options.upload(thumbEncBuffer, {
