...
const version = [2, 3e3, 1045191189];
...
const DEFAULT_CONNECTION_CONFIG = { version, browser: Browsers.macOS("Chrome"), waWebSocketUrl: "wss://web.whatsapp.com/ws/chat", connectTimeoutMs: 2e4, keepAliveIntervalMs: 15e3, logger: logger.child({ class: "baileys" }), emitOwnEvents: true, defaultQueryTimeoutMs: 6e4, customUploadHosts: [], retryRequestDelayMs: 250, maxMsgRetryCount: 3, fireInitQueries: true, auth: void 0, markOnlineOnConnect: true, syncFullHistory: true, patchMessageBeforeSending: (msg) => msg, shouldSyncHistoryMessage: ({ syncType }) => {
  return syncType !== proto.HistorySync.HistorySyncType.FULL;
}, shouldIgnoreJid: () => false, linkPreviewImageThumbnailWidth: 480, transactionOpts: { maxCommitRetries: 10, delayBetweenTriesMs: 3e3 }, eventBufferTimeoutMs: 3e4, statusBroadcastDelayMs: 1500, albumDelayMs: 1500, phashRetryEnabled: false, generateHighQualityLinkPreview: true, enableAutoSessionRecreation: true, enableRecentMessageCache: true, options: {}, appStateMacVerification: { patch: false, snapshot: false }, countryCode: "US", getMessage: async () => void 0, cachedGroupMetadata: async () => void 0, makeSignalRepository: makeLibSignalRepository };
