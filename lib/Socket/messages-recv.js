...
const makeMessagesRecvSocket = (config) => {
  ...
  // Fix: sendMessageAck ahora atrapa su propio error internamente (patrón tomado de WaSocket).
  // Antes dependía de que cada uno de los ~13 call sites le pusiera .catch() a mano; si el
  // socket se cerraba justo al mandar el ack (desconexión normal de WhatsApp), rechazaba y,
  // en el punto donde faltaba el .catch(), quedaba como promesa rechazada sin manejar.
  ...
  const handleMessage = async (node) => {
    ...
    if (encNode?.attrs.type === "msmsg") {
      ...
      // Fix: este era el único sendMessageAck sin .catch() de todo el archivo (era el punto
      // que coincidía con la traza "Connection Closed" del log). Con el fix de sendMessageAck
      // de más arriba ya no hace falta, pero lo dejamos por consistencia/defensa en profundidad.
