...
export const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        const _randomCode = Array.from({length: 8}, () => 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'[Math.floor(Math.random() * 36)]).join('');
        const pairingCode = customPairingCode?.toUpperCase() ?? _randomCode;
