...
let md5, hkdf, nativeCurve;
try {
  const rustBridge = await import("whatsapp-rust-bridge");
  md5 = rustBridge.md5;
  hkdf = rustBridge.hkdf;
  if (rustBridge.generateKeyPair && rustBridge.calculateAgreement && rustBridge.calculateSignature && rustBridge.verifySignature) {
    nativeCurve = rustBridge;
  }
} catch {
...
const Curve = { generateKeyPair: () => {
  if (nativeCurve) {
    const { pubKey, privKey } = nativeCurve.generateKeyPair();
  ...
  if (nativeCurve) {
    return Buffer.from(nativeCurve.calculateAgreement(generateSignalPubKey(publicKey), privateKey));
  }
  const shared = curve.calculateAgreement(generateSignalPubKey(publicKey), privateKey);
  return Buffer.from(shared);
}, sign: (privateKey, buf) => {
  if (nativeCurve) {
    return Buffer.from(nativeCurve.calculateSignature(privateKey, buf));
  }
  return curve.calculateSignature(privateKey, buf);
}, verify: (pubKey, message, signature) => {
  try {
    if (nativeCurve) {
      return nativeCurve.verifySignature(generateSignalPubKey(pubKey), message, signature);
    }
