...
export async function resolveJid(conn, m, target) {
    ...
    if (!m.isGroup || !m.chat) {
        ...
        // Resolver username en contactos (nuevo - soporta usernames)
        if (conn.store && conn.store.contacts && !input.includes('@')) {
            for (const contact of Object.values(conn.store.contacts)) {
                if (contact && contact.username === input) {
                    if (contact.id && /@s\.whatsapp\.net$/.test(contact.id)) {
