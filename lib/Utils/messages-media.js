...
const getHttpStream = async (url, options = {}) => {
  const response = await fetch(url.toString(), { dispatcher: options.dispatcher, method: "GET", headers: options.headers, signal: options.timeoutMs ? AbortSignal.timeout(options.timeoutMs) : AbortSignal.timeout(6e4) });
  if (!response.ok) {
