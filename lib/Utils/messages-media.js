...
const getRawMediaUploadData = async (media, mediaType, logger) => {
  ...
  // Fix: sin este listener, un error del write stream (disco lleno, descarga cortada, etc.)
  // lo autodestruye por dentro y el .write() siguiente revienta como excepción no capturable
  // ("Cannot call write after a stream was destroyed"). Guardamos el error y cortamos el loop.
...
const getHttpStream = async (url, options = {}) => {
  // Timeout por defecto subido de 60s a 15min: 60s cortaba a mitad de camino cualquier
  // descarga de media grande (videos/documentos de varios cientos de MB o ~2GB).
  // Sigue siendo configurable via options.timeoutMs si el caller quiere otro valor.
...
const encryptedStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts } = {}) => {
  ...
  // Fix: mismo problema que en getRawMediaUploadData (ver comentario ahí) — sin listener de
  // 'error' y sin chequear .destroyed antes de cada write, una descarga cortada a mitad de
  // camino (p. ej. el TypeError: terminated de fetch) dejaba el stream en carrera y terminaba
  // en "Cannot call write after a stream was destroyed" como excepción no capturada.
  let streamError = null;
