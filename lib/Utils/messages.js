...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'text')) {
        ...
        // upload cover independently with its own encryption keys
        const coverUpload = await encryptedStream(coverBuffer, 'sticker', {
            logger: options.logger,
            opts: options.options
        });
        const coverUploadResult = await options.upload(coverUpload.encWriteStream, {
            fileEncSha256B64: coverUpload.fileEncSha256.toString('base64'),
            mediaType: 'sticker',
            timeoutMs: options.mediaUploadTimeoutMs
        });
        ...
        m.stickerPackMessage = {
            ...
            thumbnailSha256: coverUpload.fileSha256,
            thumbnailEncSha256: coverUpload.fileEncSha256,
            thumbnailHeight: 96,
            thumbnailWidth: 96
        };
