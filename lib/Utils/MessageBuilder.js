...
class Button extends BaseBuilder {
  ...
  async send(jid, { ...options } = {}) {
    const msg = await this.build(jid, options);
    const bizContent = this.#isLoneSingleSelect() ? [{ tag: "list", attrs: { v: "2", type: "product_list" } }] : [{ tag: "interactive", attrs: { type: "native_flow", v: "1" }, content: [this.#buildNativeFlowNode()] }];
    await this.#client.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id, additionalNodes: [{ tag: "biz", attrs: {}, content: bizContent }], ...options });
    return msg;
  }
}
...
class AIRich extends BaseBuilder {
  ...
  addSource(sources = [], { resolveUrl = false } = {}) {
    ...
    const source = sources.map(([icon, url, text]) => ({ source_type: "THIRD_PARTY", source_display_name: text ?? "", source_subtitle: "AI", source_url: url ?? "", favicon: { url: Toolkit.resolveMedia(this.#client, icon ?? "", "image", { resolveUrl }), mime_type: "image/jpeg", width: 16, height: 16 } }));
  ...
  addImageCard(previewUrl, fullUrl = previewUrl, { resolveUrl = false } = {}) {
    ...
    this._submessages.push({ messageType: 1, gridImageMetadata: { gridImageUrl: { imagePreviewUrl: preview }, imageUrls: [{ imagePreviewUrl: preview, imageHighResUrl: full, sourceUrl: full }] } });
    this._sections.push(AIRich.newLayout("Single", { preview_image: { url: preview, mime_type: "image/jpeg", __typename: "GenAIMediaItem" }, full_image: { url: full, mime_type: "image/jpeg", __typename: "GenAIMediaItem" }, __typename: "GenAIImagePrimitive" }));
    this._mediaFallbacks.push({ type: "image", url: full || preview, caption: void 0 });
    return this;
  }
  addWidget(data = {}, { layout } = {}) {
    ...
    const widgets = items.map((item) => {
      const ctas = item.ctas ?? item.actions;
      return { header: { title: item.title, __typename: "GenAI3PExtWidgetStandardHeader" }, body: { sections: item.sections ?? [], ctas: ctas.map((cta, idx) => ({ label: cta.label ?? "", state: cta.state ?? "PENDING", kind: cta.kind ?? "OTHER", tool_call_id: cta.tool_call_id ?? String(idx).padStart(2, "0"), ...cta.toast !== false && { toast: { label: typeof cta.toast === "string" ? cta.toast : item.title, __typename: "GenAI3PExtWidgetToast" } }, __typename: "GenAI3PExtWidgetCTA" })), __typename: item.body_typename ?? "GenAI3PExtCalendarEventList" }, __typename: "GenAI3PExtWidgetPrimitive" };
    });
  ...
  async build({ forwarded = true, notification = false, includesUnifiedResponse = true, includesSubmessages = true, quoted, quotedParticipant, ...options } = {}) {
    ...
    return { messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2, botMetadata: { messageDisclaimerText: this._title, richResponseSourcesMetadata: { sources: this._richResponseSources }, ...notif } }, ...this._extraPayload, botForwardedMessage: { message: { richResponseMessage: { messageType: 1, submessages: includesSubmessages ? await waitAllPromises(this._submessages) : [], unifiedResponse: { data: includesUnifiedResponse ? Buffer.from(JSON.stringify({ response_id: crypto.randomUUID(), sections })).toString("base64") : "" }, contextInfo: { ...forward, ...qObj, ...this._contextInfo } } } } };
  }
  async send(jid, { forwarded, notification, includesUnifiedResponse, includesSubmessages, skipImageFallback = false, nativeFallback = true, quoted, ...options } = {}) {
